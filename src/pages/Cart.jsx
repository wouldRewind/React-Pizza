...
function Cart() {
    ...
    const onClearCart = () => window.confirm("Очистить корзину?") && dispatch(clearCart())
          ...
          <div className="content__items">
            {
              addedPizzas.map(obj => <CartItem key={obj.name} { ...obj } 
                ...
                totalPrice={items[obj.id].reduce((total,{ price }) => total + price,0)}
                onPizzaDelete={onPizzaDelete(obj.id)}
                onPizzaDecrease={onPizzaDecrease(obj.id)}
                onPizzaIncrease={onPizzaIncrease(obj.id)}
                />)
            }
          </div>
